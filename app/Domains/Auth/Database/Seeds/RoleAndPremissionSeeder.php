...
class RoleAndPremissionSeeder extends Seeder
{
    public function run()
    {
        ...
        $manager_role_permissions = Permission::whereStatus(config('system.status.active'))
                                            ...
                                            ->where('name', 'not like', '%Subcategory%')
                                            ->where('name', 'not like', '%Subcategorytypes%')
                                            ->where('name', 'not like', '%Type%')
        ->get();

        $sub_manager_role_permissions = Permission::whereStatus(config('system.status.active'))
        ...
        Role::findById(config('system.roles.sub_manager.id'))->givePermissionTo($sub_manager_role_permissions);
        ...
        $model_roles = [
            ...
            ['model_id' => 3, 'model_type' => 'App\Domains\Auth\Entities\User', 'role_id' => config('system.roles.sub_manager.id')],
